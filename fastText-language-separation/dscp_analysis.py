...
import os  # <-- Klasör oluşturmak için eklendi
...
try:
    ...
    print("✅ Veri Yüklendi. Satır Sayısı:", len(df))
except FileNotFoundError:
    print("❌ HATA: 'FINAL_PROJECT_DATASET.csv' bulunamadı.")
    exit()

# -------------------------------------------------------------
# 2. İSTATİSTİKSEL ANALİZLER (Sonuçlar Terminale Yazılır)
# -------------------------------------------------------------

print("\n--- İSTATİSTİK RAPORU BAŞLIYOR ---")

# A. Chi-Square (Kültür vs Duygu Türü)
print("\n[1] CHI-SQUARE TESTİ (Kültür vs Duygu Türü)")
emotion_table = pd.crosstab(df["emotion_type"], df["culture"])
chi2, p, dof, expected = chi2_contingency(emotion_table)
print(f"Chi2: {chi2:.4f}, p-value: {p:.4f}")
if p < 0.05: print("-> SONUÇ: Kültür ve Duygu Türü arasında anlamlı ilişki VAR.")
else: print("-> SONUÇ: İlişki YOK.")

# B. T-Test (Emotionality Farkı)
print("\n[2] T-TEST (Müzikal Hüzün Farkı)")
...
# -------------------------------------------------------------
# 3. GÖRSELLEŞTİRME VE KAYDETME
# -------------------------------------------------------------
print("\n📊 Grafikler 'analysis/' klasörüne kaydediliyor...")

# Grafik 1: Dağılım (KDE Plot)
...
plt.title("Emotionality Distribution (Yoğunluk Dağılımı)")
plt.xlabel("Emotionality Score (Yüksek = Hüzünlü)")
plt.legend()
...
# Grafik 3: Normalized Emotion Type (%)
...
plt.title("Kültürlere Göre Duygu Kullanım Oranları (%)")
...
# Grafik 4: NLP Confidence Distribution (NORMALİZE EDİLMİŞ VERSİYON)
plt.figure(figsize=(10,6))
sns.histplot(
    data=df, 
    x="emotion_score", 
    ...
    kde=True, 
    bins=20,
    stat="percent",      # Y ekseni artık Yüzde (%) gösterecek
    common_norm=False,   # Her kültürü kendi içinde normalize et (eşit ağırlık)
    element="step",      # Görsel karmaşayı azaltmak için içi boş step stil
    fill=True,
    alpha=0.3
)
plt.title("NLP Model Confidence Distribution (Normalized %)")
plt.ylabel("Percentage of Songs within Culture") # Y ekseni ismini düzelttik
...
plt.ylabel("Audio Emotionality Score (Yüksek = Hüzünlü)")
...
print("\n✅ TÜM İŞLEMLER TAMAMLANDI! 'analysis' klasörünü kontrol et.")